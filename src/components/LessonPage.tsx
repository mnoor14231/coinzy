...
const LessonPage: React.FC = () => {
  ...
  const { speakWithCharacter, speakGreeting, stop, isSpeaking } = useEnhancedVoice();
  const { playSound, playEmotionalFeedback, playCharacterSpecificSound } = useEnhancedSounds();
  ...
  // Greet once when character is first loaded
  useEffect(() => {
    if (!isMounted || !selectedCharacter || !audioEnabled) return;
    
    // Greet and ask to start playing
    setTimeout(() => {
      playSound('greeting', selectedCharacter);
      speakWithCharacter(
  ...
  }, [selectedCharacter, audioEnabled, isMounted, speakWithCharacter, playSound]);
  ...
  const handleConfirmAnswer = () => {
    ...
    if (isCorrect) {
      ...
      playCharacterSpecificSound(selectedCharacter, 'celebrate');
      ...
      playEmotionalFeedback('encouraging', selectedCharacter);
